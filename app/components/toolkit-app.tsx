...
const _koTranslations = {
    ...
    rangeMode: "범위 지정",
    allPages: "모든 페이지 개별",
    rangePlaceholder: "예: odd, even, first, last, middle, center, first-3, last-2, 1-3, 4-6, 7-10 또는 1 / 2 | 3-5",
    splitRangeLabel: "분할할 페이지",
    pagesPlaceholder: "예: first, start, last, end, middle, center, first-2, last-4, odd, 1-3, 4-6 또는 1/2/3",
    ...
    rotPagesPlaceholder: "예: first, middle, end, 2, 4-6, last-2 또는 1 | 3-4",
    wmText: "워터마크 텍스트",
    wmSize: "크기",
    wmOpacity: "투명도",
    wmAngle: "각도",
    ...
    pnTC: "상단 중앙",
    pnTR: "상단 우",
    deletePlaceholder: "삭제할 페이지 (예: first, middle, end, 2, 5, 8-10, last-2 또는 3/7/9)",
    deleteRangeLabel: "삭제할 페이지",
    infoInvalid: "PDF 정보를 읽을 수 없습니다.",
    pageInfoLoading: "페이지 정보 불러오는 중…",
    pageRangeFormatInvalid: "페이지 번호/범위를 입력하세요. 쉼표, 세미콜론, 줄바꿈, 슬래시, 파이프(|)로 구분하고 'all', 'odd', 'even', 'first', 'last', 'start', 'end', 'middle', 'center'를(을) 사용할 수 있습니다.",
...
const enTranslations: TranslationBundle = {
    ...
    rangeMode: "By Range",
    allPages: "Every Page",
    rangePlaceholder: "e.g. all, odd, even, first, last, middle, center, first-3, last-2, 1-3, 4-6, 7-10 or 1 / 2 | 3-5",
    splitRangeLabel: "Pages to split",
    pagesPlaceholder: "e.g. first, start, last, end, middle, center, first-2, last-4, odd, 1-3, 4-6 or 1/2/3",
    ...
    rotPagesPlaceholder: "e.g. first, middle, end, 2, 4-6, last-2 or 1 | 3-4",
    ...
    pnTC: "Top C",
    pnTR: "Top R",
    deletePlaceholder: "Pages to delete (e.g. first, middle, end, 2, 5, 8-10, last-2 or 3/7/9)",
    deleteRangeLabel: "Pages to delete",
    infoInvalid: "Unable to read PDF information.",
    pageInfoLoading: "Loading page info…",
    pageRangeFormatInvalid: "Enter page numbers or ranges (comma/semicolon/newline/slash/pipe separated). Use 'all', 'odd', 'even', 'first', 'last', 'start', 'end', 'middle', or 'center'.",
...
export default function ToolkitApp({ initialTool = "home" }: { initialTool?: View }) {
  ...
  const getRangeInputFeedback = useCallback(
    (input: string): { text: string; tone: "warning" | "error" } | null => {
      const trimmed = input.trim();
      if (!trimmed) return null;
      if (!isCurrentPdfPageInfoLoaded) return { text: t.pageInfoLoading, tone: "warning" };
      if (currentPdfPageCount === 0) return { text: t.infoInvalid, tone: "error" };
      if (!isValidPageRangeInput(trimmed)) return { text: t.pageRangeFormatInvalid, tone: "error" };
      if (!isRangeInputValidWithTotal(trimmed)) return { text: t.pageRangeOutOfBounds, tone: "error" };
      return null;
    },
  ...
  const executeValidators: Record<View, ExecuteValidator> = useMemo(
    () => ({
      ...
      rotate: ({ processing, filesCount, rotateScope, rotatePagesInput }) =>
        !processing &&
        filesCount > 0 &&
        (rotateScope === "all" || isRangeInputValidWithTotal(rotatePagesInput)),
      ...
      delete: ({ processing, filesCount, deleteInput }) => !processing && filesCount > 0 && isRangeInputValidWithTotal(deleteInput),
      info: () => false,
